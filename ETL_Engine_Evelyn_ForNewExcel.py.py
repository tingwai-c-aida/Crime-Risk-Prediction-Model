...
EXCEL_PATH = r"C:\Users\selen\OneDrive\Desktop\Evelyn\winter 2026\alberta_crime_dataset_10k.xlsx"
...
def run_pipeline():
    ...
    try:
        ...
        # 必须 100% 对齐 SQL chk_crime_category
        # --------------------------------------------------
        CRIME_CATEGORY_MAP = {
            # ✅ Excel 中真实存在的值 → SQL 允许的 canonical 值
            "assault": "Assault",
            "robbery": "Robbery",
            "theft": "Theft",
            "fraud": "Fraud",
            ...
            "b&e": "Break and Enter",
            "commercial b&e": "Break and Enter",
            "residential b&e": "Break and Enter",
            "drug offense": "Drug Offence",   # 美式 → 加式
            "drug offence": "Drug Offence"    # 已经是目标值
        }
        ...
        df["crime_category"] = (
            ...
            .str.strip()
            .str.lower()
            .map(CRIME_CATEGORY_MAP)
        )
        # --------------------------------------------------
        # 强制校验：保证 ETL 不可能突破 DB 约束
        # --------------------------------------------------
        ALLOWED_CATEGORIES = {
            "Assault",
            "Sexual Offence",
        ...
        invalid_cat = set(df["crime_category"].unique()) - ALLOWED_CATEGORIES
        if invalid_cat:
            raise ValueError(
                f"Unexpected crime_category values after standardization: {invalid_cat}"
            )
        
        df_crime = (
            df[["crime_category", "crime_subtype"]]
            .drop_duplicates()
            .reset_index(drop=True)
        )
